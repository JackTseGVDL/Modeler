...
void ModelerApplication::Init(ModelerViewCreator_f createView, 
    ...
    for (i=0; i<m_numControls; i++) {
        ...
        slider->callback((Fl_Callback*)ModelerApplication::SliderCallback, this);
    }
...
void ModelerApplication::setExtCallback_slider(void (*callback)(Fl_Slider*, void*)) {
    callback_slider = callback;
}
...
void ModelerApplication::SliderCallback(Fl_Slider *slider, void *o) {
    ModelerApplication *app = (ModelerApplication*)o;
    if (app->callback_slider != nullptr) app->callback_slider(slider, o);
